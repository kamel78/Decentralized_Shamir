...
fn from_usize(&self, input: usize) -> FieldElement<N> {
    ...
    // Convert limbs to Montgomery form
    FieldElement {
        fieldparams: &self.fieldparams,
...
//**************************************************************************************** */
    ...
    pub fn display_vec_field_elements<const N: usize>(vec: &Vec<(FieldElement<N>, FieldElement<N>)>) {
        let formatted_vec: Vec<String> = vec.iter()
            .map(|(e1, e2)| format!("({}, {})", e1.to_string(), e2.to_string()))  // Format the tuple
            .collect();
        let joined = formatted_vec.join(", ");  // Join with commas
        println!("[{}]", joined);  // Print the vector as a formatted string
    }
